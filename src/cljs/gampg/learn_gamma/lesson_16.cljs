...
(defn draw-fn [gl driver programs]
  (fn [state]
    (let [{:keys [p mv
                  ...
                  screen-data             (-> (get-data p model-mv (:vertices laptop-screen) (:normals laptop-screen) (get-in state [:framebuffer :color]) (:texture-coords laptop-screen) point-lighting-location #js[0.2 0.2 0.2]  #js[0.2 0.2 0.2]  #js[0.2 0.2 0.2])
                                              (select-keys (get-in programs [:specular :inputs]))
                                              (assoc {:tag :element-index} (:indices model)))
...
(defn make-frame-buffer [driver width height]
  (let [color {:width      width
    ...
    (gd/frame-buffer driver {:color (gd/texture driver color)
                             :depth (gd/render-buffer driver depth)})))
